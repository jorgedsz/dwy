...
import api, { portalAPI, twilioAPI } from '../../services/api';
...
export default function ClientDetailPage() {
  ...
  const fetchLastCall = async () => {
    ...
    try {
      const res = await twilioAPI.getLastCall(id);
      setLastCall(res.data.lastCall);
    } catch (err) {
      setLastCallError(err.response?.data?.error || 'Failed to fetch call data');
    } finally {
  ...
  const handleTwilioSave = async () => {
    ...
    try {
      await api.put(`/clients/${id}`, { twilioAccountSid: twilioSid, twilioAuthToken: twilioToken });
      setShowTwilioForm(false);
      setTwilioSid('');
      setTwilioToken('');
      load();
    } catch (err) {
      console.error('Save Twilio creds error:', err);
    } finally {
  ...
  const handleTwilioDisconnect = async () => {
    try {
      await api.put(`/clients/${id}`, { twilioAccountSid: '', twilioAuthToken: '' });
      setLastCall(null);
      setLastCallError(null);
      load();
    } catch (err) {
      console.error('Disconnect Twilio error:', err);
    }
  };
